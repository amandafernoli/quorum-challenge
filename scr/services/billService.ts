 ...
 */
const getBillVotes = function (
  votesIdsBillFiltered: number[],
  allVotes: VoteResult[],
  voteType: number
): VoteResult[] {
  return allVotes.filter(
    (vote) =>
      votesIdsBillFiltered.includes(vote.vote_id) && vote.vote_type == voteType
  );
};
...
const billsVotes = function (
  ...
  return bills.map((bill) => {
    ...
    return {
      id: bill.id,
      title: bill.title,
      supporter_count: votesYes.length,
      opposer_count: votesNo.length,
      primary_sponsor: getSponsorById(bill.sponsor_id, legislators),
    };
  });
};
